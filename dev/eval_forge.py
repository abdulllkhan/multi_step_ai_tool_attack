"""End-to-end eval_attack with the CURRENT attack.py on a board GGUF model. Compares
forge-multipost (forge=True) vs single-post (forge=False) by real replay SCORE."""
...
which=sys.argv[1]; budget=float(sys.argv[2]) if len(sys.argv)>2 else 150.0
forge=(sys.argv[3]!="0") if len(sys.argv)>3 else True
fac=(gptoss_factory if which=="gpt_oss" else gemma4_factory)()
opts=AttackEvalOptions(run_config=AttackRunConfig(time_budget_s=budget,max_steps=10**9,max_tool_hops=EVALUATION_DEFAULT_MAX_TOOL_HOPS),env_seed=123,guardrail_factory=OptimalGuardrail,attack_config={"forge":forge,"forge_n":8})
t=time.time()
f=eval_attack(m.AttackAlgorithm,budget,fac,"sandbox",options=opts)
sc=score_attack(f); N=len(f)
# total EXFIL predicates across findings = total posts scored
posts=sum(sum(1 for p in x.get("predicates",[]) if p.get("predicate")=="EXFILTRATION") for x in f)
print(f"[{which}] forge={forge} budget={budget:.0f}s: SCORE={sc:.3f} findings={N} exfil_posts={posts} posts/finding={posts/max(N,1):.1f} ({time.time()-t:.0f}s)",flush=True)
os._exit(0)
